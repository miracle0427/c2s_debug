...
if __name__ == '__main__':
    ...
    data_frame = policy_db.policies

    data_frame.to_csv("c2s_rs.csv")

    output_dp_dict = {} 
    output_dp_dict["Sources"] = []
    output_dp_dict["Destinations"] = []

    for line_num in range(len(data_frame)):
        output_dp_dict["Sources"].append(data_frame["Sources"][line_num])
        destinations = ""
        for destination in data_frame["Destinations"][line_num]:
            destinations += '^' + destination.router
        output_dp_dict["Destinations"].append(destinations)

    output_dp = pd.DataFrame(output_dp_dict)
    output_dp.to_csv("./dst_src_rs.csv")
