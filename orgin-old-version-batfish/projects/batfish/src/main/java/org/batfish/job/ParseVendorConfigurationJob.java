...
import org.batfish.representation.HWConfigJSON.HWJSONConfiguration;
...
public class ParseVendorConfigurationJob extends BatfishJob<ParseVendorConfigurationResult> {
  ...
  public ParseVendorConfigurationResult call() throws Exception {
    ...
    */
    ...
    // set the format by myself
    format = ConfigurationFormat.CISCO_IOS;
    /*
    try {
    ...
    */
    HWJSONConfiguration hwvc = new HWJSONConfiguration();
    hwvc._jsonStr = _fileText;
    hwvc.setJsonData(hwvc._jsonStr);
    hwvc.setFilename(_filename);
    //vc = extractor.getVendorConfiguration();
    hwvc.setVendor(format);
    vc = (VendorConfiguration)hwvc;
    vc.setVendor(format);
